...
Available commands:
    ...
    import-lon              Try to import entries from Lon
...
To update the repository in the future, the variable needs to be set again and nix needs to be configured accordingly to be able to fetch it (see the [`netrc-file` option](https://nix.dev/manual/nix/2.24/command-ref/conf-file#conf-netrc-file)).
Environment example:
```console
